...
namespace Graph_lib
{
  ...
  {
    return Point(c.center().x + c.radius() * cos(315 * PI() / 180), c.center().y + c.radius() * sin(315 * PI() / 180));
  }

  Point Graph_lib::se(const Graph_lib::Circle& c) // bottom-right
  {
    return Point(c.center().x + c.radius() * cos(45 * PI() / 180), c.center().y + c.radius() * sin(45 * PI() / 180));
  }

  Point Graph_lib::sw(const Graph_lib::Circle& c) // bottom-left
  {
    return Point(c.center().x + c.radius() * cos(135 * PI() / 180), c.center().y + c.radius() * sin(135 * PI() / 180));
  }

  Point Graph_lib::nw(const Graph_lib::Circle& c) // top-left
  {
    return Point(c.center().x + c.radius() * cos(225 * PI() / 180), c.center().y + c.radius() * sin(225 * PI() / 180));
  }
  ...
  {
    return Point(e.center().x + e.major() * cos((-45) * PI() / 180), e.center().y + e.minor() * sin((-45) * PI() / 180));
  }
  ...
  {
    return Point(e.center().x + e.major() * cos(135 * PI() / 180), e.center().y + e.minor() * sin(135 * PI() / 180));
  }

  Point Graph_lib::nw(const Graph_lib::Ellipse& e) // top-left
  {
    return Point(e.center().x + e.major() * cos(225 * PI() / 180), e.center().y + e.minor() * sin(225 * PI() / 180));
  }

  Box_text::Box_text(Point p, std::string ss):
    Rectangle{ p, ss.length() * 10, 40 },
    text{ Point(p.x + 5, p.y + 30), ss }
  {
    add(p);
  ...
  bool Graph_lib::on_line(Point p1, Point p2, Point pp)
  {
    int dx = p2.x - p1.x;
    int dy = p2.y - p1.y;
    int dx_p = pp.x - p1.x;
  ...
  void Graph_lib::Striped_closed_polyline::draw_lines() const
  {
    ...
    for (int y = y_min + d; y < y_max; y += d)
    {
      ...
      if (intersection(Point(x_min, y), Point(x_max, y), point(number_of_points() - 1), point(0), pp))
      {
        if (pp != pp_back)
        {
          v_lines.push_back(pp);
